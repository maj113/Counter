...
uint64_t bench(const char* data, uint64_t dataSize, uint64_t numIterations, bool useOptimized, bool singleThreaded) {
    ...
    for (uint64_t i = 0; i < numIterations; ++i) {
        ...
        uint64_t count = useOptimized ? opt_count_parallel(
        data, data + dataSize, 10, singleThreaded) : std::count(data, data + dataSize, 10);
...
int main() {
    ...
    double improvement_single = ((double)std_single_cumulative - (double)opt_single_cumulative) / (double)std_single_cumulative * 100.0;
    double improvement_multi = ((double)std_single_cumulative - (double)opt_multi_cumulative) / (double)std_single_cumulative * 100.0;

    cout << "Improvement over std::count: single threaded: " << fixed << setprecision(2) << abs(improvement_single) << "% multi threaded: " << fixed << setprecision(2) << abs(improvement_multi) << "%" << endl;

    cout << "Times faster over std::count: single threaded: " << fixed << setprecision(2) << (double)std_single_cumulative / (double)opt_single_cumulative << "x multi threaded: " << fixed << setprecision(2) << (double)std_single_cumulative / (double)opt_multi_cumulative << "x" << endl;
