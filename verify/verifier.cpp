...
int main() {
    constexpr std::array<int, 46> data_sizes{
        ...
        77777777, 88888888, 100000000, 123456789, 125000000, 150000000, 987654321, 1500000000, 2147483647}
    };
    constexpr char target = 'x', filler = 'a';

    for (uint64_t size : data_sizes) {
        char* data = static_cast<char*>(_aligned_malloc(size, 32));
        
        std::memset(data, filler, size);

        for (uint64_t count = 1; count <= size; count *= 2) { 
        ...
        jump:
            ...
            uint64_t rParallel = counter(data, data + size, target, false);
            uint64_t rSingle = counter(data, data + size, target,  true);
